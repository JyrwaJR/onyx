...
- [ ] **Step 1: Set `status: 'pending'` on the optimistic message**
...
- [ ] **Step 2: Verify `Message` type accepts `'pending'`**
...
- [ ] **Step 1: Remove the `onMutate`/`onError`/`onSettled` optimistic cache manipulation**
...
- [ ] **Step 3: Verify no other references to `previousMessages`/`temp-` remain**
...
- [ ] **Step 1: Remove `sending` prop from `MessageInput` usage in ChatScreen**
...
- [ ] **Step 2: Remove `sending` from `MessageInputProps` and its guards**
